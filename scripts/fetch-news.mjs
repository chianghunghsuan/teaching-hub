...
async function summarizeForClassroom(candidates) {
  const prompt = [
    ...
    "請從候選新聞中挑出 4 到 6 則最適合課堂分享的內容。優先：學生升學、科系探索、職涯方向、日常生活、公共健康、食安、科學、科技、AI、機器人、半導體、能源、天氣、自然現象、太空、重大國際局勢。",
    ...
    "盡量涵蓋不同面向：學生與教育至少 1 則、科技或職涯至少 1 則、科學或太空至少 1 則，其餘從國際、公共健康、經濟中挑最有資訊量的。",
    "過濾血腥、八卦、未證實傳聞、犯罪細節、純股價漲跌、純投資炒作、政治口水，以及不適合課堂的內容。",
    "每則都要有深度，不能只復述新聞。要把事件放進更大脈絡：背後機制、牽涉的產業或制度、和哪些相關議題相連、對學生未來的意義，並可旁徵博引相關知識與常見的數據趨勢，讓學生產生連結。",
    ...
    "關鍵數據：列 3 到 6 點，每點一行以「- 」開頭。來源有的具體數字照列；補充的外部數據若沒把握，寫成『（趨勢，可查證：來源名）』，不要假裝精確；真的沒有數據就寫『- 來源未提供可直接採信的具體數字』。",
    "反思結論：至少 3 句，用老師口吻講出這則新聞真正的重點、限制或機會，以及你希望學生記得的一句話。",
    "延伸討論：列 2 點，每點一行以「- 」開頭，格式為『問題？→ 參考方向：…（可查：真實來源）』，一定要給思考方向或簡短答案，並指名可查資料的來源（例如主計總處、教育部統計處、勞動部、104 或 1111 人力銀行年度報告、國際組織報告、原始論文）。不要只丟問題不給方向。",
