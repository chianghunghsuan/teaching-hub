...
async function summarizeForClassroom(candidates) {
  console.log(
    "CANDIDATES:\n" +
      candidates
        .map((c) => `${c.article_index}. [${c.source || ""}] ${c.title}`)
        .join("\n"),
  );
  const prompt = [
    ...
    "【最高優先的選稿規則，務必遵守】候選中只要出現升學或考試相關新聞（含分科測驗、學測、指考、放榜、五標、頂標、級分、落點分析、填志願、錄取、繁星、申請入學），你一定要至少選 1 則放進結果，這是對補習班學生最有用、優先級最高的類別，絕對不可以因為科技新聞比較多就略過它。",
